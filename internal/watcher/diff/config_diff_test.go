...
func TestBuildConfigChangeDetails(t *testing.T) {
	...
	expectContains(t, details, "remote-management.panel-github-branch: main -> dev")
...
func TestBuildConfigChangeDetails_SecretsAndCounts(t *testing.T) {
	oldCfg := &config.Config{
		SDKConfig: sdkconfig.SDKConfig{
			APIKeys: []sdkconfig.ClientAPIKey{{Key: "a"}},
		},
		AmpCode: config.AmpCode{
			UpstreamAPIKey: "",
		},
	...
	newCfg := &config.Config{
		SDKConfig: sdkconfig.SDKConfig{
			APIKeys: []sdkconfig.ClientAPIKey{{Key: "a"}, {Key: "b"}, {Key: "c"}},
		},
		AmpCode: config.AmpCode{
...
func TestBuildConfigChangeDetails_FlagsAndKeys(t *testing.T) {
	oldCfg := &config.Config{
		...
		RemoteManagement:       config.RemoteManagement{DisableControlPanel: false, PanelGitHubRepository: "old/repo", PanelGitHubBranch: "main", SecretKey: "keep"},
		SDKConfig: sdkconfig.SDKConfig{
			RequestLog:                 false,
			ProxyURL:                   "http://old-proxy",
			APIKeys:                    []sdkconfig.ClientAPIKey{{Key: "key-1"}},
	...
	newCfg := &config.Config{
		...
		SDKConfig: sdkconfig.SDKConfig{
			...
			APIKeys:                    []sdkconfig.ClientAPIKey{{Key: " key-1 "}, {Key: "key-2"}},
...
func TestBuildConfigChangeDetails_AllBranches(t *testing.T) {
	oldCfg := &config.Config{
		...
		SDKConfig: sdkconfig.SDKConfig{
			RequestLog: false,
			ProxyURL:   "http://old-proxy",
			APIKeys:    []sdkconfig.ClientAPIKey{{Key: " keyA "}},
		},
	...
	newCfg := &config.Config{
		...
		SDKConfig: sdkconfig.SDKConfig{
			RequestLog: true,
			ProxyURL:   "http://new-proxy",
			APIKeys:    []sdkconfig.ClientAPIKey{{Key: "keyB"}},
		},
	...
	expectContains(t, changes, "remote-management.panel-github-branch: main -> dev")
